...
const Profile: FC<Props> = ({ user }) => {
  ...
  const removeCourse = (courseId: string) => {
    setExcludedCourses((prevExcluded) => [...prevExcluded, courseId]);
  };
      ...
      {active === 3 && (
        <div className="w-full pl-7 px-2 800px:px-10 800px:pl-8 mt-[80px]">
          <div className="grid grid-cols-1 gap-[20px] md:grid-cols-2 md:gap-[25px] lg:grid-cols-3 lg:gap-[25px] 1500px:grid-cols-4 1500px:gap-[35px] mb-12 border-0">
            {courses &&
              courses
                .filter((item: any) => !excludedCourses.includes(item._id))
                .map((item: any, index: number) => (
                  <CourseCard
                    item={item}
                    key={index}
                    isProfile={true}
                    onRemove={() => removeCourse(item._id)}
                  />
                ))}
          </div>
          {courses.length === 0 && (
            ...
            {user.role === "admin"
              ? "You don't have any courses to approve!"
              : user.role === "instructor"
              ? "You don't have any approved courses!"
